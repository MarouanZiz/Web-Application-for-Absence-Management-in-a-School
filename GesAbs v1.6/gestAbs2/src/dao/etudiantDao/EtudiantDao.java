...
public class EtudiantDao {
     ...
     public boolean delEtd(String cne , String cin) {
		...
		public ArrayList<Etudiant> getAllEtd(int idEtab){
    	 try {
			...
			PreparedStatement st=c.prepareStatement("select * from etudiant where idEtab=? group by idNiv asc");
			st.setInt(1, idEtab);
			ResultSet res=st.executeQuery();
		while(res.next()) {
			Etudiant et=new Etudiant();
     ...
     public String addEtudiant(Etudiant et) {
    		...
    		try {
    			PreparedStatement st = c.prepareStatement("insert into etudiant(cne,cin,nom,prenom,datenais,login,password,email,sexe,lieunais,idetab,idniv) values(?,?,?,?,?,?,?,?,?,?,?,?)");
    			st.setString(1,et.getCne());
    			st.setString(2,et.getCin());
    			st.setString(3,et.getNom());
    			st.setString(4,et.getPrenom());
    	...
    	public String getImg(String cne,String cin) {
			try {
			PreparedStatement st = c.prepareStatement("select photo from etudiant where cne=? and cin=?");
		    st.setString(1, cne);
		    st.setString(2, cin);
			ResultSet rs = st.executeQuery();
			String photo = null;
