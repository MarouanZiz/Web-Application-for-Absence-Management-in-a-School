...
<div style="overflow-x:auto">
 	  <table  class="etudiants">
			...
			<tr>
			<td>${e.getCnp()}</td><td>${e.getCin()}</td><td>${e.getNom() }</td><td>${e.getPrenom()}</td><td>${e.getDateNais()}</td><td>${e.getLogin()}</td><td>${e.getPsw()}</td><td>${e.getEmail() }</td>
			<td>${e.getSexe()}</td><td>${e.getTele()}</td><td>${etab.getEtabliById(e.getIdEtab()).getNomEtab() }</td><td><a href="../../supEnseignant?cnp=${e.getCnp()}&cin=${e.getCin()}"><button type="button"  class="btn btn-danger">Supprimer</button></a></td><td><a href="../../miseEnseignant?cnp=${e.getCnp()}&cin=${e.getCin()}"><button type="button"  class="btn btn-dark">Modifier</button></a></td>
			</tr>
			</c:forEach>
			</table>
			</div>
