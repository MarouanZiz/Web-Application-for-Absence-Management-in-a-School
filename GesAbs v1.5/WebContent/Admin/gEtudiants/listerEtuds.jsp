...
<a  href="../espaceAdmin.jsp">Acceuil</a>
  ...
  <a class="active" href="listerEtuds.jsp">Lister tout</a>
...
<div class="col-sm-9">
<div class="content">
<h4 class="titliste">Gestion des Etudiants > Liste des Etudiants</h4>
<div style="overflow-x:auto;">
 	<table  class="etudiants">
			...
			<td>${e.getSexe()}</td><td> ${e.getLieuNais()}</td><td>${etab.getEtabliById(e.getIdEtab()).getNomEtab() }</td><td>${niv.getNivById(e.getIdNiv()).getNomNiveau()}</td><td><a href="../../supEtudiant?cne=${e.getCne()}&cin=${e.getCin()}"><button type="button"  class="btn btn-danger">Supprimer</button></a></td><td><a href="../../miseEtudiant?cne=${e.getCne()}&cin=${e.getCin()}"><button type="button"  class="btn btn-dark">Modifier</button></a></td></tr>
			</c:forEach>
			</table>
			</div>
			<b>${msg}</b>
